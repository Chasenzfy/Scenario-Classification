...
from model_batch import PredictionGGNN_CLASS
...
def train_test(args, epochs, train_graph, val_graph, test_graph, model, optimizer):
    ...
    for epoch in range(1, epochs + 1):
        ...
        '''
        print(logits[torch.tensor([0, 1, 2])])
        print(logits.argmax(dim=1))
        print(train_graph["y"])
        print(train_acc)

        print("train_graph.x.shape[0]: " + str(train_graph.x.shape[0]))
        '''
...
def main(epochs, lr, num_layer, emb_dim,seed):
    ...
    train_data = torch.load("train1.pt")
    val_data = torch.load("val1.pt")
    test_data = torch.load("test1.pt")
    ...
    model = PredictionGGNN_CLASS(in_feats=16, out_feats=16, n_steps=num_layer, aggregate_type="mean", num_cls=16).to(device)
...
def logic(seed):
    clas = LogisticRegression(solver='sag',max_iter=2000,random_state=seed)
    train_data = torch.load("train1.pt")
    test_data = torch.load("test1.pt")
    val_data = torch.load('val1.pt')
    train_keys = train_data['graph']['x']
...
def split_batch(test_list=[], val_list=[], featuretype='tfidf', filename='textfeature.pt'):
    ...
    combine(test_list, "test1.pt", featuretype, filename)
    combine(val_list, "val1.pt", featuretype, filename)
    size = combine(train_list, "train1.pt", featuretype, filename)
    return size
    ...
    for i in range(0,6):
        ...
        for layer_size in mlp_layer_list:
            for seed in range(0, 11):
                ...
                filebase = './newsplit5/mlpclass/mlp_'+str(i)+'_'
        ...
        for layer_size in mlp_layer_list:
            for seed in range(0, 11):
                logger.warning("start logic2")
                logger.warning(layer_size)
                filebase = './newsplit5/logic/logic_'+str(i)+'_'
                filename = str(layer_size[0]) + '_' + str(len(layer_size)) + '_' + str(seed)+ '.txt'
                filename = filebase + filename
                with open(filename, 'w+') as file:
                    sys.stdout = file
